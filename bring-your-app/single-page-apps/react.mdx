...
This documentation provides a step-by-step guide to prepare a Single Page Application (SPA) that was built using ReactJS for deploying it in any cloud using LocalOps.
...
Assume that the site we built is being served by nginx via the domain `example.com`. Whenever an API call is made to `example.com/api`, the nginx server acts as a reverse proxy server and forwards the request to the backend server. You can use reverse proxying to navigate the traffic that is inbound to nginx. The reverse proxy server also acts as a mask, hiding the real backend from the outside world.
...
After running the command, visit `http://localhost:3000` to see the React application running inside the Docker container.
...
`js-example-spa` is an example TODO app built with. This project is dockerized and the images are published to Amazon Elastic Container Registry (ECR).

## Next Steps

Once Docker is ready, the next steps will be:

- [Publishing the Docker images](/bring-your-app/first-steps/have-docker#publish-docker-images-to-aws-ecr)
- [Packaging your app with Helm](/bring-your-app/first-steps/have-docker#package-a-helm-chart)
...
You might also have a backend associated. Check out the [tutorials](/category/tutorials) to learn more about integrating the app with your backend.
